...
\textbf{Coordinate convention.}  The camera now uses an engineering
$Z$-up convention ($XY$ ground plane, $+Z$ upward), matching common
CAD/CAE coordinate systems.  All preset views and the default
orientation rotate around the world $Z$ axis for yaw, not $Y$.
...
Mouse drag produces two incremental rotations per frame, both applied
in the camera's \textbf{local space} (post-multiplication):
\begin{itemize}
  \item \textbf{Yaw} $\Delta\alpha$ --- rotation about the camera's
        own up axis $Y$.  Applied as a post-multiplication
        $\mathbf{q}' = \mathbf{q} \, R_Y(\Delta\alpha)$ so the view
        spins left/right consistently with the screen drag,
        \emph{regardless of the camera's current orientation}.
  \item \textbf{Pitch} $\Delta\beta$ --- rotation about the camera's
        local $X$ axis.  Applied as a post-multiplication
        $\mathbf{q}' \leftarrow \mathbf{q}' \, R_X(-\Delta\beta)$,
        where the sign preserves the ``drag up = look up''
        convention.
\end{itemize}
...
\[
  ...
  = \mathrm{normalize}\!\left(
      \mathbf{q}_{\text{old}} \;\cdot\; R_Y(\Delta\alpha) \;\cdot\; R_X(-\Delta\beta)
    \right).
\]

\paragraph{Why local-space yaw, not world-Y.}
The original implementation pre-multiplied the yaw rotation about the
\emph{world} $Y$ axis ($R_Y \, \mathbf{q}$) to keep the horizon level.
However, once the camera is rolled upside-down (e.g.\ after pitching
past $90^\circ$), the world-$Y$ yaw rotates in the \emph{opposite}
direction to the screen drag: dragging left turns the view right.
Switching to local-space yaw ($\mathbf{q} \, R_Y$) makes the on-screen
response always match the drag direction, at the cost of the horizon
no longer staying level after a roll---an acceptable trade-off for an
interactive inspection camera.
...
The parameter names \texttt{drag\_x}/\texttt{drag\_y} (rather than
\texttt{delta\_yaw}/\texttt{delta\_pitch}) reflect that the inputs are
screen-space drag deltas, not angular increments---the conversion to
rotation angle happens inside \texttt{orbit} via the caller-supplied
sensitivity factor.
...
\begin{tabular}{lll}
\toprule
Preset & Orientation & Description ($Z$-up) \\
\midrule
\texttt{view\_front} & $\mathbf{q} = R_X(+\tfrac{\pi}{2})$ & camera at $-Y$, looking at $+Y$, $Z$ up \\
\texttt{view\_top}   & $\mathbf{q} = I$ & camera at $+Z$, looking at $-Z$, $Y$ screen-up \\
\texttt{view\_right} & $\mathbf{q} = R_Z(-\tfrac{\pi}{2})\,R_X(-\tfrac{\pi}{2})$ & camera at $+X$, looking at $-X$, $Z$ up \\
\texttt{view\_iso}   & $\mathbf{q} = R_Z(-0.785)\,R_X(-0.615)$ & isometric 3/4 view \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}
  ...
  \item \textbf{Orbit} (left mouse) --- \texttt{camera.orbit($-\Delta x \cdot s$,
        $-\Delta y \cdot s$)} composes incremental local-space quaternion
        rotations.
  \item \textbf{Pan} (right mouse) --- translates \texttt{target}
        along camera-local $X$ and $Y$.  Screen $y$ grows downward,
        so the drag delta is negated: \texttt{pan($-\Delta x \cdot s$,
        $-\Delta y \cdot s$)} makes the object follow the cursor
        (drag up $\to$ object up, drag right $\to$ object right).
  \item \textbf{Zoom} (scroll) --- scales \texttt{distance} with a
        scene-radius-relative clamp
        $[0.02\,r,\,500\,r]$ and resynchronises the near/far clip
        planes (Section~\ref{sec:clip}).
...
\section{Zoom-Synchronised Clip Planes}
\label{sec:clip}

Initially \texttt{near} / \texttt{far} were computed once in
\texttt{fit\_to\_aabb} and never touched again, while \texttt{zoom()}
only changed \texttt{distance} (clamped to the fixed range
$[0.1, 500]$).  Consequently the object vanished whenever
\[
  d < \text{\texttt{near}} \quad (\text{zoom in}) \qquad\text{or}\qquad
  d > \text{\texttt{far}} \quad (\text{zoom out}),
\]
with $d$ the camera distance --- the reported ``object disappears at
extreme zoom'' bug.

The fix introduces a \texttt{scene\_radius} $r$ (half the fitted AABB
diagonal, stored by \texttt{fit\_to\_aabb}).  Every zoom step
recomputes the clip planes from the new distance:
\begin{align}
  d &\leftarrow \operatorname{clamp}\!\bigl(d\,(1-\delta),\;
        0.02\,r,\; 500\,r\bigr), \\[2pt]
  \text{\texttt{near}} &\leftarrow \max\!\bigl(d - 2r,\;
        10^{-3} d,\; 10^{-4}\bigr), \\[2pt]
  \text{\texttt{far}} &\leftarrow \max\!\bigl(d + 2r,\; 10\bigr).
\end{align}
The nearest possible surface point is $d - r$ (bounding sphere), so
$\text{\texttt{near}} = d - 2r$ keeps a full-radius margin in front of
the geometry --- robust against a slightly underestimated
\texttt{scene\_radius} (e.g.\ \texttt{colorbar3d} sets \texttt{distance}
manually and never fits).  When the camera moves inside the bounding
sphere ($d - 2r < 0$) the near plane falls back to a small fraction of
$d$, which stays in front of the camera.  Because both planes follow
$d$, the ratio $\text{\texttt{far}}/\text{\texttt{near}}$ stays bounded
and depth precision is preserved across the whole zoom range.

A regression test (\texttt{zoom\_keeps\_object\_inside\_clip\_planes})
zooms across the reachable range for radii
$\{0.05, 1, 1.73, 40\}$ and asserts the bounding sphere never leaves
$[\text{\texttt{near}}, \text{\texttt{far}}]$.
...
\begin{itemize}
  ...
  \item Zooming in or out to any reachable distance keeps the object
        inside the frustum (Section~\ref{sec:clip}); the previously
        reported disappearance at extreme zoom is fixed and covered by
        a unit test.
